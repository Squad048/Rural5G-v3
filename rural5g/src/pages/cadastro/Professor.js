...
const Professor = () => {
    ...
    function onSubmit(ev) {
        ...
        axios.post('http://localhost:8080/api/professores/signup', values)
            .then((response) => {
                navigate("/");
            });
    }
    return (
        <section className="container my-5 rounded-4" style={{ backgroundColor: "rgb(119, 231, 119)" }}>
            <h1 className='text-center'>Cadastrar Professor</h1>
            <form className="w-75 p-5 mb-5" onSubmit={onSubmit}>
                ...
                <div className="form-floating mb-3">
                    <input type="text" name="materia" className="form-control rounded-3"
                        id="floatingInputL" onChange={onChange}/>
                    <label htmlFor="floatingInput">Digite sua materia</label>
                </div>
